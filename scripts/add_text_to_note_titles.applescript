...
This is an AppleScript for manipulation of a set of Evernote notes.  When run, it modifies the titles of all selected notes by adding text entered by the user.  The text can be added to the beginning or the end of the note title.
...
on run {}
	tell application "Evernote"
		...
		if selected_notes is not equal to {} then
			set prefix to ""
			set suffix to ""
			set add_result to display dialog "text to be added" default answer Â
				"" buttons {"Cancel", "Prepend", "Append"} Â
				default button Â
